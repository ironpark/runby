...
멀티플렉서 서버는 처음 붙은 클라이언트의 환경을 유지하고 **이미 실행 중인 pane의 환경은 갱신할 수 없습니다.** 그래서 `Multiplexer()`가 잡히면 환경에서 파생된 계층 — `Terminal`, 에이전트, 러너 — 의 `Confidence`를 `probable`로 낮춥니다. 단 살아 있는 조상으로 확증된 계층(`AncestorPID != 0`)은 지금 실행 중임이 증명되므로 낮추지 않습니다. SSH는 다릅니다 — 터미널이 다른 머신에 있을 수 있다는 뜻이지만 값이 낡은 것은 아니므로, 신뢰도를 낮추는 대신 `RemoteSSH` 계층의 존재로 그 사실을 표현합니다.
...
잔존 위험은 터미널 축에만 걸리지 않습니다. 오래 사는 서버는 처음 시작될 때의 에이전트·러너 마커도 나중 pane에 물려주므로, 위의 강등이 그 두 축에도 똑같이 적용됩니다. CI 축은 예외입니다 — CI 잡은 멀티플렉서의 저장된 환경을 거쳐 실행되는 것이 아니기 때문입니다.
