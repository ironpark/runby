...
멀티플렉서 서버는 이전 클라이언트의 환경을 오래 유지할 수 있습니다. `runby`는 살아 있는 조상으로 확증되지 않은 터미널·에이전트·러너의 신뢰도를 낮춰 표시합니다. 중요한 판단에서는 [상위 프로세스 정보](process.md)를 함께 사용하세요. CI 축은 멀티플렉서의 저장된 환경을 거쳐 실행되는 계층이 아니므로 이 강등에서 제외됩니다.
