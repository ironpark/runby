...
`Interactive`는 프롬프트를 띄우고 응답을 받을 수 있는 형태라는 뜻일 뿐, **사용자가 직접 명령을 호출했다는 증거는 아닙니다.** 에이전트나 서브에이전트도 PTY를 할당할 수 있습니다. "사람이 보고 있는가"를 알고 싶다면 `TTY.Interactive`가 아니라 `Found()`가 거짓인지를 보십시오.
