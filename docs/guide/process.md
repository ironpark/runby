...
일반적인 기능 분기에는 환경 기반 축만으로 충분합니다. 감지된 에이전트나 터미널이 **현재도 살아 있는지** 확인해야 할 때 이 축을 사용하세요.
...
에이전트가 감지되고 그 실행 파일이 조상으로 살아 있으면 `Agent.AncestorPID`가 채워집니다.
