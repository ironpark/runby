...
| `product_type` | `agent_harness`, 여러 하위 agent를 관리하는 `agent_orchestrator`, 또는 agent를 내장·연결하는 `agent_host` |
...
`product_type`은 코드의 `Kind`와 1:1로 대응합니다. `agent_harness`는 `KindHarness`, `agent_orchestrator`는 `KindOrchestrator`, `agent_host`는 `KindHost`입니다. `KindHost`는 어떤 애플리케이션이 터미널을 소유하는지만 증명하므로 `Result.IsAgent()`에 포함되지 않습니다.
