...
`runby`는 에이전트·CI·터미널·실행 도구·원격 다섯 축을 보고합니다. WSL 자신은 원격 축의 `RemoteWSL`(`RemoteKindEnvironment`) 계층으로 보고되며, 나머지 축에는 서로 다른 방식으로 영향을 미칩니다.
