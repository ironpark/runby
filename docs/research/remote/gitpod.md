...
- **에이전트 축** — Ona로의 리브랜딩 이후 회사는 "에이전트를 위한 미션 컨트롤"을 표방하지만, 이번 조사에서 접근 가능했던 공식 문서 범위 안에서는 Gitpod/Ona 워크스페이스가 특정 에이전트 실행을 표시하는 고정 이름의 환경변수를 문서화한 근거를 찾지 못했습니다. 워크스페이스 안에서 별도 에이전트 CLI(Claude Code, Codex 등)를 실행하면 그 CLI 자체의 마커가 나타날 뿐이며, 이는 `docs/research/agents/`의 각 문서가 이미 다루는 영역입니다. 그래서 이 문서의 `executes_agents`는 빈 배열입니다.
