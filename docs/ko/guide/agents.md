...
| Charm Crush | `AgentCrush` | `harness` | `multi-vendor` | `CRUSH=1` (`AGENT`·`AI_AGENT`는 값이 `crush`일 때만 보조 evidence) |
| Roo Code | `AgentRooCode` | `harness` | `multi-vendor` | `ROO_ACTIVE=true` (터미널 소유 보조 신호) |
| OpenHands | `AgentOpenHands` | `harness` | `multi-vendor` | `AI_AGENT=openhands` (대소문자 무시; hook 컨텍스트는 `Extra`) |
