...
다른 모든 문서([`docs/terminals/README.md`](../terminals/README.md) 등)가 "OpenSSH 기본값은 `TERM`과 `LANG`/`LC_*` 외에는 전달하지 않는다"고 인용하는 근거가 바로 이 절입니다. 아래는 그 주장을 man 페이지 원문으로 직접 검증한 결과입니다.
...
- 이 `LC_*` 채널이 넓게 열려 있다는 사실이 바로 [`docs/terminals/iterm2.md`](../terminals/iterm2.md)가 경고하는 문제의 근본 원인입니다 — iTerm2는 터미널 식별자를 `LC_TERMINAL`/`LC_TERMINAL_VERSION`이라는 `LC_*` 네임스페이스 변수에 담기 때문에, 이 변수들은 SSH가 다른 모든 것을 막아도 배포판 기본 `SendEnv LC_*` 규칙에 걸려 아무 특별한 설정 없이 원격으로 전파됩니다.
...
`runby`는 에이전트·CI·터미널 세 축을 보고합니다. SSH 경계를 넘을 때 이 셋이 받는 영향은 방향이 다릅니다.
